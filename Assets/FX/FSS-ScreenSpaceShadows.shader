...
Shader "Hidden/FSS-ScreenSpaceShadows" {
Properties {
...
uniform sampler3D global_NoiseTex3D;
...
fixed4 frag_hard (v2f i) : SV_Target {
	...
	//FSS
	FSS_DECLARE_SHADOWNUDGE_COORD(wpos, vpos);
	shadowCoord.xyz += shadowCoordNudge;

	

    //1 tap hard shadow
    fixed shadow = UNITY_SAMPLE_SHADOW(_ShadowMapTexture, shadowCoord);
	//fixed shadow = mainImage(shadowCoord);

	//shadow = mainImage(i.pos.xy, wpos.wx  * 0.01);
